...
- `cmaps`: coil sensitivity maps.  Pass `nothing` (default) to auto-estimate
  ...
  equal to `downsample`, to use pre-computed maps.  Must already reside on
  the same device as `data`.  Ignored for single-coil data.
...
function estimate_delay(
    ...
    elseif !is_singlecoil && cmaps === nothing
        ...
        # Validate user-provided cmaps
        cmaps isa AbstractVector || throw(ArgumentError(
            "`cmaps` must be a Vector of arrays (one per coil), got $(typeof(cmaps))."))
        length(cmaps) == Ncoil || throw(ArgumentError(
            "`cmaps` length ($(length(cmaps))) does not match the number of coils ($Ncoil)."))
        for (ic, cmap) in enumerate(cmaps)
            cmap isa AbstractArray || throw(ArgumentError(
                "`cmaps[$ic]` must be an AbstractArray, got $(typeof(cmap))."))
            size(cmap) == downsample || throw(ArgumentError(
                "`cmaps[$ic]` has size $(size(cmap)), expected $downsample (must match `downsample`)."))
        end
    end
